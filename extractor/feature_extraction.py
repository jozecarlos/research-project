...
def create_feature_extractor(architecture, input_size):
    ...
    if architecture == 'Inception3':
        ...
        feature_extractor = DenseNet201Feature(input_size, weights)
        ...
        feature_extractor = VGG19Feature(input_size, weights)
    elif architecture == 'ResNet50':
        feature_extractor = ResNet50Feature(input_size, weights)
    else:
        raise Exception(
            'Architecture not supported! Only support Full Yolo, Tiny Yolo, MobileNet, DenseNet201, SqueezeNet, VGG19, ResNet50, and Inception3 at the moment!')
...
class DenseNet201Feature(BaseFeatureExtractor):
    ...
    def __init__(self, input_size, weights):
        ...
        desennet201 = DenseNet201(input_shape=(224, 224, 3), include_top=False)
        if weights:
            desennet201.load_weights(weights)

        x = desennet201(input_image)
        self.feature_extractor = Model(input_image, x)
...
class VGG19Feature(BaseFeatureExtractor):
    """docstring for ClassName"""

    def __init__(self, input_size, weights):
        vgg19 = VGG19(input_shape=(input_size, input_size, 3), include_top=False)
        if weights:
